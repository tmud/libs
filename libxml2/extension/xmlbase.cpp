...
void FormatTree(xmlNodePtr root)
{
    ...
    while (root)
    {
        ...
        if (root->type == XML_TEXT_NODE && root->content)
        {
            ...
            // удал€ем все узлы, которые представл€ют собой только пробелы и переводы строк
...
void xmlInit()
{
    // »нициализируем парсер
    xmlInitParser();

    // —оздаем пустой документ и корневой элемент - Root,
    // куда в дальнейшем будем загружать xml файлы, создава€ общее xml-дерево	
